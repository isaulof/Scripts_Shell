...
# DATA: 05/11/2025
# VERSÃO: 1.5 (Adaptado para ambientes virtuais)
#
# --- INÍCIO DO SCRIPT ---
...
if command -v curl &> /dev/null; then
    ...
    # Se o curl não estiver instalado, não exibe nada.
    : 
fi
...
# Ajustado para não mostrar "-1" se o apt não existir ou falhar.
if command -v apt &> /dev/null; then
    ...
    # Não exibe nada se o 'apt' não estiver presente
    :
fi
